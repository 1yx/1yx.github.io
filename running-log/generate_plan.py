...
================================================================================
一、整体结构（层级：Session > Mesocycle > Set > Day > Workout）
================================================================================
  Session ：一张工作表 = 一种课表（Aerobic / Threshold / Marathon）。
  Mesocycle   ：每 4 个小周期(microcycle)为一块。
  Set     ：块内第 1/2/3 个小周期为 Normal，第 4 个为 Cutback(减量)。
  ...
  小周期序号 cycle 与 (Mesocycle, Set) 的换算：
      mesocycle = (cycle-1)//4 + 1 ； set = (cycle-1)%4 + 1
...
【Aerobic Session】28 周期，2026/6/1 起。里程随周期递增(ramp，单位 km)：
    ...
    Day1 上午：26+Δ Long Run（Normal）/ 18+Δ Medium Long Run（Cutback）
    Day2 上/下午：6km Recovery ×2
    Day3 上午：21+Δ Medium-Long Run（Normal）/ 13+Δ Medium Run（Cutback）
...
【Threshold Session】12 周期(3 块)，2026/9/21 起。每块配速递进 −10″：
    ...
    Cutback(Set4)：照抄 Aerobic 周期4 减量，并把 Day4 下午的 Recovery 挪到 Day3 下午。
...
【Marathon Sheet】18 周期(4 块 + 末尾 taper)，2026/11/8 起（到 2027/1/18）。
    ...
    Day1 下午：留空
    Cutback(Set4)：照抄 Threshold Set4（即上面那套减量）。
...
def is_cutback(cycle):
    """Set 4 = 减量周期。"""
    return cycle % 4 == 0


# ============================================================================
# 3. 训练内容（按 Session > Mesocycle > Set > Day > Workout）
#    每个 resolver 给定 (cycle, mesocycle, set, day, slot) 返回该格文本，None=留空。
# ============================================================================
...
def aerobic_workout(cycle, day, slot):
    ...
    cutback = is_cutback(cycle)
    if day == 1:
        if slot == "AM":
            return f"{18 + delta}km Medium Long Run" if cutback else f"{26 + delta}km Long Run"
        return None                       # Day1 下午休息
    if day in (2, 4):                     # Day2 / Day4：上下午都是 Recovery
        return "6km Recovery"
    if day == 3:
        if slot == "AM":
            return f"{13 + delta}km Medium Run" if cutback else f"{21 + delta}km Medium-Long Run"
...
def threshold_workout(cycle, mesocycle, set_, day, slot):
    """Threshold 某(周期,块,Set,天,上午/下午)的训练内容。"""
    if set_ == 4:                         # Set4 = 减量
        return cutback_workout(day, slot)
    if day == 1:
        if slot == "AM":
            return f"2km Warm Up + {THRESHOLD_INTERVAL[set_]}@{THRESHOLD_PACE[mesocycle]}"
        return f"5km Warm Up + 3*1km@{THRESHOLD_NEXT_PACE[mesocycle]}"
    if day == 2:
...
def marathon_workout(cycle, mesocycle, set_, day, slot):
    """Marathon 某(周期,块,Set,天,上午/下午)的训练内容。"""
    if set_ == 4:                         # Set4 = 减量（同 Threshold Set4）
        return cutback_workout(day, slot)
    if (mesocycle, set_) not in MARATHON_DAY1:    # taper(块5) → 留空
        return None
    if day == 1:
        return MARATHON_DAY1[(mesocycle, set_)] if slot == "AM" else None
    if day in (2, 3):
        return "10km Medium Run" if slot == "AM" else "6km Recovery"
    if day == 4:
...
def workout_fill_rgb(session_name, cycle, day, slot, text):
    ...
    _, set_ = cycle_to_mesocycle_set(cycle)
...
# workout 解析函数签名统一为 (cycle, mesocycle, set, day, slot) -> 文本或 None
SESSIONS = [
...
def build_session(wb, session):
    ...
    for cycle in range(1, n + 1):
        mesocycle, set_ = cycle_to_mesocycle_set(cycle)     # 该周期对应的 Mesocycle / Set
        ...
        for day in range(1, DAYS_PER_CYCLE + 1):
            ...
            for slot, row in (("AM", ar), ("PM", pr)):
                text = workout_fn(cycle, mesocycle, set_, day, slot)
