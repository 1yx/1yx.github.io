...
从 0.5.1 到 1.0.0，一个大版本更新。specify-cli 是 [GitHub spec-kit](https://github.com/github/spec-kit) 的 CLI 工具。我去查 Release Notes 准备升级，发现仓库的 Release 页面上根本没有 1.0.0。最新版本还在 0.x 的范围里。
...
`uv tool list --outdated` 的默认行为是拿本地包名去 PyPI 索引查询。因为 `1.0.0` 在语义化版本上确实大于 `0.5.1`，uv 产生了误报。如果我盲目执行了 `uv tool upgrade --all`，它会把我的 spec-kit 替换成一个完全不同的 PyPI 包。
...
在我的案例中：
- 我的工具：`specify-cli` v0.5.1，从 `git+https://github.com/github/spec-kit.git` 安装
- PyPI 上的包：`specify-cli` v1.0.0，不同作者的不同项目

uv 看到 "specify-cli"，问 PyPI "specify-cli 的最新版本是什么？" PyPI 回答 "1.0.0"。uv 比较 1.0.0 > 0.5.1，报告有更新。但这个更新在源仓库中根本不存在。它是一个幻影。

### 两种失败模式：假阳性 vs 未找到

这个问题根据 PyPI 上是否存在同名包，有两种表现形式：

**假阳性**（我遇到的情况）：PyPI 上有一个同名但不同的包，版本号更高。`uv tool list --outdated` 报告了一个在 Git 源中不存在的升级。执行 `uv tool upgrade` 可能会把你的工具替换成错误的包。
...
识别出 Git 源工具后，检查更新需要三步。以下方法假设使用 GitHub 托管的仓库。
...
uv 比 pipx 强的地方：速度。uv 用 Rust 编写，大多数操作比 pipx 快 10-100 倍。依赖解析更好，缓存更激进，工具隔离更干净。对于 PyPI 源工具，uv 严格更优。
...
完整实现在我 `tui-devflow-config` 仓库中：

- **检查脚本**：[devflow-update.fish](https://github.com/1yx/tui-devflow-config/blob/main/fish/.config/fish/functions/devflow-update.fish) — 分类工具、通过对应渠道检查过期、展示结果
- **升级脚本**：[devflow-upgrade.fish](https://github.com/1yx/tui-devflow-config/blob/main/fish/.config/fish/functions/devflow-upgrade.fish) — 使用分离策略执行升级
