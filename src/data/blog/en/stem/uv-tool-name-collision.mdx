...
```
specify-cli v0.5.1 [latest: 1.0.0]
```

A major version bump, from 0.5.1 to 1.0.0. For specify-cli, which is [GitHub's spec-kit](https://github.com/github/spec-kit) CLI tool. I went to check the release notes before upgrading, and found no 1.0.0 release on the repository's Release page. The latest was somewhere in the 0.x range.
...
In my case:
- My tool: `specify-cli` v0.5.1, installed from `git+https://github.com/github/spec-kit.git`
- PyPI package: `specify-cli` v1.0.0, a different project by a different author

uv sees "specify-cli" and asks PyPI "what's the latest version of specify-cli?" PyPI answers "1.0.0." uv compares 1.0.0 > 0.5.1 and reports an update available. But the update doesn't exist in the source repository. It's a phantom.
...
specify-cli v0.5.1 [required: git+https://github.com/github/spec-kit.git]
...
The full implementation lives in my `tui-devflow-config` repository:

- **Check script**: [devflow-update.fish](https://github.com/1yx/tui-devflow-config/blob/main/fish/.config/fish/functions/devflow-update.fish) — classifies tools, checks outdated via appropriate channels
- **Upgrade script**: [devflow-upgrade.fish](https://github.com/1yx/tui-devflow-config/blob/main/fish/.config/fish/functions/devflow-upgrade.fish) — executes upgrades using the split strategy
